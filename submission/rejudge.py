...
@click.option(
    ...
    default="",
    help=
    "the submission id you want to submit , will generate one on runtime if not given"
)
@click.option("-c",
              "--count",
              "count",
              type=int,
              default=1,
              help="the time of testing rejudge")
@click.option("--seq",
              "sequential",
              type=bool,
              default=True,
              help="wheather to test it in specify order")
@click.option("--cfg",
              "config",
              type=click.File("r"),
              help="see github wiki for more detail")
@click.option(
    "-d",
    "--delay",
    "delay",
...
def rejudge(submission_id: str, count: int, sequential: bool, config,
            delay: float, max_time: float, fname: str):
    '''
